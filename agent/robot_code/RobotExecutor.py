...
# DELTAS
PICKUP_DELTA_PRE = [-0.025, 0.02, 0.30]
PICKUP_DELTA_CORE = [-0.025, 0.02, 0.17]
PICKUP_DELTA_POST = [-0.025, 0.02, 0.30]
...
class RobotActionExecutor:
    def __init__(self, available_table_markers, objects, loc_marker_pos):
        ...
        self.available_table_markers = available_table_markers

	# add table as an obstacle
    	self.add_table(loc_marker_pos)
    ...
    def add_table(self, loc_marker_pos):
        initial_bound = 0.015
        self.scene = moveit_python.PlanningSceneInterface('/base_link')
        self.scene.addBox("table", 1, 1, 0.5, loc_marker_pos[self.available_table_markers[0]][0] + 0.5, loc_marker_pos[self.available_table_markers[0]][1] + 0.3, loc_marker_pos[self.available_table_markers[0]][2] - 0.3)
    ...
    def _update_pose_position(self, pose, deltas):
        '''
        Please finish this to make each action more compact
        '''
        pass
    ...
    def execute_putdown(self, loc_marker_positions): 
        ...
        self.group.clear_pose_targets()
        pose_target = geometry_msgs.msg.Pose()
        if len(self.available_table_markers) > 0:
            free_table_marker = self.available_table_markers.pop(-1)
