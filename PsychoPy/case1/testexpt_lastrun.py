...
expName = 'testexpt'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/akulka16/Downloads/Psychopy/case1/testexpt.psyexp',
...
instrText = visual.TextStim(win=win, name='instrText',
    text="In the bottom left corner, robot's goal is specified.\n\nPress any key to start the experiment",
    font='Arial',
...
for thisTrial in trials:
    ...
    # keep track of which components have finished
    trialComponents = [movie, image_1, movie_2]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        frameRemains = 2 + 3- win.monitorFramePeriod * 0.75  # most of one frame period left
        if movie.status == STARTED and t >= frameRemains:
            movie.setAutoDraw(False)
        
        # *image_1* updates
        if t >= 2 and image_1.status == NOT_STARTED:
            # keep track of start time/frame for later
            image_1.tStart = t
        ...
        if image_1.status == STARTED:  # only update if drawing
            ...
            image_1.setImage(imageFile, log=False)
            image_1.setSize((0.15, 0.2), log=False)
        
        # *movie_2* updates
        if t >= 5 and movie_2.status == NOT_STARTED:
            # keep track of start time/frame for later
            movie_2.tStart = t
